...
function Navbar() {
    ...
    const handleLogout = () => {
        axios.get('https://api.rc.credenz.in/api/logout', {
            // axios.get('https://b56b-106-193-237-218.ngrok-free.app/api/logout', {
                headers: {
                    'Authorization': `${localStorage.getItem('jwt')}`
                }
            })
    ...
    return (
        <div className='navbar h-17 w-full px-[5vw] text-white flex justify-between items-center bg-transparent '>

            <div className="nav-left flex justify-center items-center">
                <img src={rclogo} alt="" className='h-[60px] w-[100px]' />
            </div>
            <div className="nav-right text-[17px] font-normal justify-center items-center gap-[20px] hidden md:flex font-red-hat-display">
                {tokenExists && (
                    <>
